...
# (! кэшировать, и еще что-нибудь)¬ лоб очень долго, нужно это дело как-то оптимизировать,
# далее после того как привели в норм форму построить облака слов

# подумать куда лучше записывать нормализованные данные,
# возможно df дл€ этого новый, и его конвертнуть в csv
...
# 3. посчитать tf-idf дл€ каждого слова из доков
...
# 4. ƒл€ каждого слова: cложить все метрики по всем документам

def summ_all_metrics_for_words():
    print('')

# 5. ќтранжировать слова по метрике tf-idf
...
# —равнить результаты со списком наиболее частотных слов (без tf-idf)
